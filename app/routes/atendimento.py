...
def processar_atendimento():
    ...
    try:
        ...
        # ---------------- Fluxos CLIENTE (Google + detecção simples) ------------------
            ...
            # 1) DETECÇÃO (fallback rápido por keywords)
            intent = None
            try:
                txt = (mensagem or "").lower()
                if any(k in txt for k in ["demitid", "pensão", "divida", "dívida", "processo", "juizado", "direito"]):
                    intent = "relato_caso"
                elif any(k in txt for k in ["agendar", "consulta", "horário", "reunião", "agenda"]):
                    intent = "agendar_consulta_cliente"
                elif any(k in txt for k in ["rg", "cnh", "comprovante", "documento", "anexo"]):
                    intent = "enviar_documento_cliente"
                else:
                    intent = "fluxo_nao_detectado"
            except Exception:
                intent = "fluxo_nao_detectado"
            ...
            if intent == "relato_caso":
                ...
                resposta_texto = (
                    "✅ Seu **caso** foi **registrado na planilha** do escritório. Vamos acompanhar seu **processo**."
                    if ok else
                    "⚠️ Tente novamente: não consegui registrar seu caso na **planilha**."
                )
                ...
                if event_id:
                    ...
                    try:
                        ...
                        pass
                    resposta_texto = f"✅ **Evento criado** no **Calendar**. Link: {event_link} | 📧 **Email enviado** pelo **Gmail**."
                else:
                    resposta_texto = "⚠️ Não consegui criar o evento no **Calendar** agora. Podemos tentar outro horário?"
